(function() {
	...
	hpms.init = function() {
		map = avlmap.Map({id:'#hpms-map', minZoom: 4})
			.height(d3.select('.body').node().offsetWidth-200)
	...
	function calcStrokeDomain(cb) {
	    ...
	    d3.json("http://api.availabs.org/hpms/hpms/aadt")
    ...
    function showRouteData(r) {
    	...
    	if (r.properties.route) {
			...
			url = 'http://api.availabs.org/hpms/hpms/'+r.properties.route;
    ...
    function LayerCache(maxSize) {
		...
		function newLayer(id) {
			var url = "http://lor.availabs.org:1331/" + id + "/{z}/{x}/{y}.json",

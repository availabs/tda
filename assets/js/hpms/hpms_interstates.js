(function() {
	...
	hpms.init = function() {
		map = avlminimap.CanvasMap()
			...
			.height(window.innerHeight-210-margin.top)
			.clearColor('#99a');

		quadtree = d3.geom.quadtree()
			.extent([[0,0],[d3.select('.body').node().offsetWidth, window.innerHeight-margin.top]]);
		quadtreeRoot = quadtree([]);

		voronoi = d3.geom.voronoi()
			.clipExtent([[0,0],[d3.select('.body').node().offsetWidth,window.innerHeight-30]]);
		...
		d3.json('/data/us_states.json', function(error, data) {
			...
			d3.json('http://api.availabs.org/hpms/hpms/interstates_by_state', function(error, data) {
				var domain = [];
	...
	function addInterstate(datum) {
		LOADING++;
		XHRcache[datum] = d3.json('http://api.availabs.org/hpms/hpms/'+datum+'/interstate_geo', function(error, data) {
